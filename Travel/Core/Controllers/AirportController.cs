namespace Travel.Core.Controllers
{
    ...
    public class AirportController : IAirportController
	{
		...
		public string RegisterBag(string username, IEnumerable<string> bagItems)
		{
            ...
            List<Item> itemsOfPassenger = new List<Item>();
            foreach (var item in bagItems)
            {
                var currentItem = itemFactory.CreateItem(item);
                itemsOfPassenger.Add((Item)currentItem);
            }
			var bag = new Bag(passenger, itemsOfPassenger);
		...
		{
			var passenger = this.airport.GetPassenger(username);
            var trip = (Trip)this.airport.Trips.FirstOrDefault(t => t.Id == tripId);

			var checkedIn =trip.Airplane.Passengers.Any(p => p.Username == passenger.Username);
            if (checkedIn)
			{
